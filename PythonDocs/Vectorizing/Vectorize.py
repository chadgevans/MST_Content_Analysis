...
titles = []
corpus = []
for fn in os.listdir('presSpeeches'):
    f = open('presSpeeches/'+fn,'r')
    description = f.readline()
    description = description.strip()
    speaker, title = description.split('\t')
...
f = open('speechVectors.txt','w')
f.write('speaker\ttitle\tyear\t' + '\t'.join(terms) + '\n')
for i in xrange(vs.shape[0]):
    f.write('"%s"\t"%s"\t' % (speakers[i],titles[i]))
    year = re.findall(r'(\d*)\)',titles[i]) # this works most of the time
    if len(year)>0:
        year = year[0]
    else:
        year = 'NA'
